...
def process_language(LANG):
    ...
    if not os.path.isfile(LANGFILE) or not os.path.isfile(ENGLISH_LANGFILE):
        print(f"Language file for {LANG} or English not found. Skipping...")
        return None
    ...
    for m in matcher:
        ...
        lang_matcher = lang_pattern.search(translations)
        if lang_matcher:
...
with zipfile.ZipFile(zip_filename, 'w') as zipf:
    for LANG in tqdm(languages, total=len(languages), desc='Processing files'):
        OUTPUT_FILE = process_language(LANG)
        if OUTPUT_FILE is not None:
            zipf.write(OUTPUT_FILE, f"weapons_{os.path.basename(OUTPUT_FILE)}")
            os.remove(OUTPUT_FILE)
